<?
	...
	if(isset($_POST['save']) && !empty($_POST['save']) && $_POST['save'] == 1){
		...
		$ins_womst->setValues("'$newNum','$today','$serviceType','$equipment','$meter','$remarks'
							,'$labor','$misc','$parts','$disc','$tax','$subTotal','$totalCost','$isWarranty','$isBackJob'
							,'$sys_UserID','$today'");
		...
		if(!empty($arrParts)){
			...
			for($i=0;$i<count($partsItem);$i++){
				...
				$ins_wodtl->setValues("'$newNum','$pItem[0]','$pItem[3]','$pItem[4]'");
